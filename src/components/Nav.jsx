              ...
              <div className="border-t border-gray-200 py-6 px-4 space-y-6 ">
                {navigation.links.map((link) => (
                  <div className="flow-root" key={link.id} link={link}>
                    <Link
                      onClick={() => setOpen(false)}
                      className="-m-2 p-2 block text-[#2858dc] transition-all duration-200 ease-in-out hover:bg-[#2858dc] hover:text-white font-medium "
                      href={link.href}
                    >
                      {link.name}
                    </Link>
      ...
      <header className="relative bg-white">
        ...
        <div className="bg-white border-b ">
          <div className="container mx-auto flex gap-8 items-center justify-end text-xs font-medium text-[#2858dc] px-4 sm:px-6 lg:px-8 h-10">
            <Link rel="noopener" target="_blank" href="tel:+237651421052">
              {" "}
              <span className="flex gap-2 font-semibold text-xs items-center">
                {" "}
                <FaPhoneVolume /> Call Us Now!
              </span>
            </Link>
            <Link href="https://wa.me/+237651421052">
              {" "}
              <span className=" flex gap-1 font-semibold text-xs items-center ">
                <FaWhatsapp /> Whatsapp{" "}
              </span>
            </Link>
          </div>
            ...
            <div className="h-16 flex items-center">
              ...
              <div className="ml-4 flex lg:ml-0">
                <Link href="/" className="flex items-center gap-2">
                  <Image
                    className="w-6 h-8 lg:w-12 lg:h-12 object-contain "
                    src={logoImg}
                    alt="logo"
                    width={80}
                    height={80}
                ...
                <div className="hidden lg:flex lg:flex-1 lg:items-center text-center lg:justify-end lg:space-x-8 ">
                  {navigation.links.map((link) => (
                    <div className="flow-root" key={link.id} link={link}>
                      <Link
                        onClick={() => setOpen(false)}
                        className="-m-2 p-2 block text-[#2858dc] font-medium text-sm uppercase transition-all duration-200 ease-in-out hover:text-[#2858dc] tracking-wide underline-offset-8 hover:underline "
                        href={link.href}
                      >
                        {link.name}
                      </Link>
